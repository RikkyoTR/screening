...
st.caption("項目ごとに複数枚の写真・動画を選択して判定します。（高精度化のため3回アンサンブル解析を実行します）")
        ...
        if not any([v_ohsq, v_rear_sq, p_ankle, p_thoracic, p_aslr, p_sheet]):
            ...
            with st.spinner("AIが3回の試行測定を行い、平均値を算出中...（10〜20秒ほどかかります）"):
                try:
                    ...
                    【高精度解析指示】
                    画像・動画を分析するにあたり、測定誤差や判定のブレを防ぐため、**各写真・動画に対して全項目3回の内部測定を実施し、その平均値を最終判定値として採用してください**。

                    ※重要フォーマット＆角度・距離の計算・測定規則※
                    - 写真判定の各項目について、**複数枚の写真がある場合は写真1枚ごとにインデント（箇条書きの配下）で段落分けして表示**してください。
                    
                    - **足関節背屈** の測定について:
                      1. **距離測定（cm）**: 写真内に写っている「メジャー（定規）」の目盛りを拡大認識してスケール基準とし、**壁から「足のつま先」までの距離（cm）**を測定してください。ほとんどの場合「靴下」を着用しているため、靴下の厚み（約0.3〜0.5cm程度）を考慮・微調整した上で実際の素足つま先位置を推定測定してください。
                      2. **角度測定（度）**: 併せて**「足首の屈曲（背屈）角度」**（すねと足底のなす角度、または足首関節の屈曲度）も測定してください。
                      3. **表示形式**: 理由や解説文は一切記載せず、**【距離数値（cm） / 足首角度（°）】の数値表示のみ**を行ってください（例: 12.5cm / 38°）。

                    - **A/P SLR** の測定角度について:
                      1. 「股関節（大転子付近）」と「挙上している側の外くるぶし（外果）」を結んだ直線と「水平線」とのなす【足側の挙上角度】（例: 75°）を3回試行測定し、平均値を求めてください。
                      2. その後、**【180° − (3回測定した平均挙上角度)】** の計算を行ってください。
                    ...
                    ### 📸 写真判定（可動域・チェック項目）
                    - **足関節背屈**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      - 写真1 (ファイル名): [距離数値 (cm) / 足首角度 (°)]  (例: 12.5cm / 38°)
                      - 写真2 (ファイル名): [距離数値 (cm) / 足首角度 (°)]  (例: 11.0cm / 35°)
                    - **胸椎回旋**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      - 写真1 (ファイル名): [3回平均の数値または状態]
                      - 写真2 (ファイル名): [3回平均の数値または状態]
                    - **A/P SLR**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                    ...
                    # Gemini API 呼び出し（429/503エラー対策のリトライ処理付き）
                    response = None
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            ...
                            if ("503" in err_msg or "429" in err_msg) and attempt < max_retries - 1:
                                time.sleep(4 * (attempt + 1)) # 4秒、8秒と待機して再接続
                                continue
                            else:
                                raise req_err

                    st.success("✅ 3回アンサンブル解析・判定完了")
                    ...
                    st.error(f"判定エラーが発生しました: {e}\n\n※APIのリミット（429/503）にかかりました。約1分ほど時間を空けてから再度お試しください。")
