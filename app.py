        ...
        if not any([v_ohsq, v_rear_sq, p_ankle, p_thoracic, p_aslr, p_sheet]):
            ...
            with st.spinner("AIが指定された項目別に解析中..."):
                try:
                    ...
                    - **A/P SLR** の測定角度について:
                      * **「股関節（大転子付近）」と「挙上している側の外くるぶし（外果）」を結んだ直線**と、**「水平線」**とのなす**【足側の角度（床上からの挙上角度）】**を読み取って測定・算出してください（頭側の補角・錯角ではありません）。
                      * 表示の際は理由や説明文は一切書かず、**写真ごとの角度数値（例: 75°）のみ**を表示してください。
                    ...
                    - **胸椎回旋**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      ...
                      - 写真2 (ファイル名): [数値または状態]
                    - **A/P SLR**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      - 写真1 (ファイル名): [足側とのなす挙上角度 (例: 75°)]
                      - 写真2 (ファイル名): [足側とのなす挙上角度 (例: 65°)]
                    - **手書きシート/その他**:
