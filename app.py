        ...
        if not any([v_ohsq, v_rear_sq, p_ankle, p_thoracic, p_aslr, p_sheet]):
            ...
            with st.spinner("AIが指定された項目別に解析中..."):
                try:
                    ...
                    ※重要フォーマット規則※
                    - 写真判定の各項目（足関節背屈、胸椎回旋、A/P SLR、手書きシート）について、**複数枚の写真がある場合は写真1枚ごとにインデント（箇条書きの配下）で段落分けして表示**してください。
                    - **A/P SLR** の項目については、理由や状態の解説文は一切書かず、**写真ごとの角度数値（例: 75°）のみ**を表示してください。
                    ...
                    - **胸椎回旋**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      ...
                      - 写真2 (ファイル名): [数値または状態]
                    - **A/P SLR**: [ ⭕ 正常 / ❌ 要改善 / 未提出 ]
                      - 写真1 (ファイル名): [角度数値のみ (例: 75°)]
                      - 写真2 (ファイル名): [角度数値のみ (例: 65°)]
                    - **手書きシート/その他**:
